...
function createYoutubeAudioFinanceReportTable($tableName, $conn)
{
	$returnArr = array();
	if (empty($tableName)) {
...
// 	}
// }
function insertYRAudioFinanceReportInfo($filePath, $tableName, $conn)
{
	$returnArr = array();
	if (empty($tableName)) {
		return setErrorStack($returnArr, 4, getErrMsg(4)." Tablename to create cannot be empty", null);
...
////////////////////////////////////Auto Assign helpers////////////////////////////////////////
function autoAssignChannelCOMapforYTAudio($tableName="", $fieldSearchArr=null, $contentType="", $conn)
{
	$returnArr = array();
	$res = array();
...
//echo $updatecasestring;exit;
	...
	foreach($allco as $k=>$y){
		$bulkupdatecasestring.=" when (assetChannelID='{$k}' or channelID ='{$k}') and {$whereClause} then '{$y[$contentType]}'";
