...
function createYoutubeVideoClaimReportTable($tableName, $conn)
{
    ...
    $createYoutubeTableQuery = "CREATE TABLE {$tableName}  (
        ...
        `video_title` varchar(150) DEFAULT NULL,
        `autoassign_steps` varchar(10) DEFAULT NULL,
        `holding_percentage` decimal(10,2) DEFAULT 30,
		 PRIMARY KEY (id),
         INDEX i (assetChannelID),
         INDEX contentType (contentType),
         INDEX channelID (channelID),
...
function createYoutubeVideoClaimReportTable2($tableName, $conn)
{
    ...
    $createYoutubeTableQuery = "CREATE TABLE {$tableName}  (
        ...
        @unlink("polo_create_table.txt");
        file_put_contents("polo_create_table.txt", $createYoutubeTableQuery);
        @chmod("polo_create_table.txt", 0777);
    $createAmazonTableQueryResult = runQuery($createYoutubeTableQuery, $conn);
    // printArr($createAmazonTableQueryResult);exit;
...
function autoAssignContentOwnerChannelCOMapStep1($tableName, $conn)
{
    ...
    // 
      ...
      $client_youtube_sharesQuery = "UPDATE {$tableName} inner join channel_co_maping on {$tableName}.assetChannelID = channel_co_maping.assetChannelID  SET {$tableName}.holding_percentage = channel_co_maping.client_youtube_shares   where  channel_co_maping.CMS ='{$nd_type}' and country='US' and partnerRevenue>0 ";
...
function generateActication_youtube_ecommerce_paid_features_Reportv3($sourcetable, $desinationtable, $conn)
{
    ...
    if (noError($updateQueryResult)) {
        ...
        AVG(holding_percentage) AS holding_percentage     FROM     {$sourcetable}  r where r.country='US' and r.holding_percentage > 0     GROUP BY         content_owner ) r ON     m.content_owner = r.content_owner SET     m.holding_percentage = r.holding_percentage";
