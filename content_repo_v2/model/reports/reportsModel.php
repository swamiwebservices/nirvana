...
function createYoutubeReportTable($tableName, $conn)
{
	$returnArr = array();
	if (empty($tableName)) {
	 ...
	 $createYoutubeTableQuery = "CREATE TABLE {$tableName}  (
							  ...
							  `youtubeRevenueSplitAuction` decimal(18,8) NOT NULL,
							  `youtubeRevenueSplitReserved` decimal(18,8) NOT NULL,
							  `youtubeRevenueSplitPartnerSoldYoutubeServed` decimal(18,8) NOT NULL,
							  `youtubeRevenueSplitPartnerSoldPartnerServed` decimal(18,8) NOT NULL,
							  `youtubeRevenueSplit` decimal(18,8) NOT NULL,
							  `partnerRevenueAuction` decimal(18,8) NOT NULL,
							  `partnerRevenueReserved` decimal(18,8) NOT NULL,
							  `partnerRevenuePartnerSoldYouTubeServed` decimal(18,8) NOT NULL,
							  `partnerRevenuePartnerSoldPartnerServed` decimal(18,8) NOT NULL,
							  `partnerRevenue` decimal(18,8) NOT NULL,
							   ...
							   INDEX compositeforvalidate (videoID,channelID)
							)";
...
function insertReportInfo($filePath, $tableName, $conn)
{
	$returnArr = array();
	if (empty($tableName)) {
		return setErrorStack($returnArr, 4, getErrMsg(4)." Tablename to create cannot be empty", null);
...
////////////////////////////////////Auto Assign helpers////////////////////////////////////////
function autoAssignChannelCOMap($tableName="", $fieldSearchArr=null, $contentType="", $conn)
{
	$returnArr = array();
	$res = array();
...
function autoAssignPrevMonths($tableName, $sourceTableName, $conn)
{
	$returnArr = array();
	$res = array();
...
//	$a3 = runQuery("SET sql_log_bin = 0;", $conn);
	...
	$autoAssignPreviousMonthsQuery = "UPDATE  {$tableName}  t1 INNER JOIN  {$sourceTableName}  t2 ON  t1.videoID=t2.videoID AND LOWER(t1.contentType)=LOWER(t2.contentType) AND t1.content_owner=''  SET t1.content_owner=t2.content_owner";
