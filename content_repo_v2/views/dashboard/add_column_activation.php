 ...
 ///   $subject = "test ".time();
  ...
  //  echo "<br> checkg mail ";
   // sendMail($to, $subject, $message);
    ...
    $getClientInfoQuery = "SHOW TABLES LIKE '%activa%'";
    ...
    while ($row =  mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            $tableName = $v;
         //   $truncateTableQuery = "ALTER TABLE {$tableName} ADD COLUMN   `gst_percentage` decimal(10,2) DEFAULT 0,  `holding_percentage` decimal(10,2) DEFAULT 0, `final_payable_with_gst` varchar(50)  DEFAULT NULL ";
            $truncateTableQuery = "ALTER TABLE {$tableName} ADD COLUMN  `gst_percentage` DECIMAL(10,2) DEFAULT 0 AFTER `status`, ADD `holding_percentage` DECIMAL(10,2) DEFAULT {US_HOLDING_PERCENTAGE} AFTER `gst_percentage`, ADD `final_payable_with_gst` VARCHAR(50) NULL AFTER `holding_percentage` ";
