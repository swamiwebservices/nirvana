    ...
    if (!noError($userInfo)) {
        ...
        if (empty($userInfo)) {
            ...
            if ($userInfo[$email]["status"]!=1) {
                ...
                $logMsg = "Attempting to get count of all monthly rate of saavan.";
                $logData["step4"]["data"] = "4. {$logMsg}";
                
                //set the search array based on get parameters
                $distributorSearchArr = array("ndtype"=>'saavan');                
                ...
                if (!noError($allDistributorsCount)) {
                    ...
                    $returnArr["errCode"] = 5;
                    $returnArr["errMsg"] = getErrMsg(5)." Error fetching Months rate saavan count details.";
                } else {
                    ...
                    $logMsg = "Got all monthly rate of saavan count for page: {$page}. Now getting all monthly rate of saavan info";
                    ...
                    if (!noError($allDistributorsInfo)) {
                        ...
                        $logMsg = "Couldn't fetch all monthly rate of saavan info: {$allDistributorsInfo["errMsg"]}";
                        $logData["step4.1"]["data"] = "4.1. {$logMsg}";
                        $logsProcessor->writeJSON($logFileName, $logFilePath, $logData, $initLogs["activity"]);
                        
                        $returnArr["errCode"] = 5;
                        $returnArr["errMsg"] = getErrMsg(5)." Error fetching monthly rate of saavan details.";
                    } else {
                        $logMsg = "Got all monthly rate of saavan data for page: {$page}";
                        $logData["step4"]["data"] = "4. {$logMsg}";
                                    ...
                                    <table class="table table-bordered table-condensed">
                                        ...
                                        <tbody>
                                            ...
                                            <td><?php echo $distributorDetails["month"]; ?>-<?php echo $distributorDetails["year"]; ?></td>
                                                ...
                                                <td><?php echo $distributorDetails["Ad_Supported_Revenue"]; ?></td>
                                                <td><?php echo $distributorDetails["Subscription_Revenue"]; ?></td>
