...
//$id = "";
if (isset($_POST["id"])) {
    $id = cleanQueryParameter($conn, cleanXSS(urldecode($_POST["id"])));
}

if (empty($id)) {
    $logMsg = "Activity downlaod report id field empty: ".json_encode($_POST);;
    $logData["step2.1"]["data"] = "2.1. {$logMsg}";
    $logsProcessor->writeJSON($logFileName, $logFilePath, $logData, $initLogs["activity"]);

    $returnArr["errCode"] = 4;
    $returnArr["errMsg"] = getErrMsg(4)." id   cannot be empty";
    echo(json_encode($returnArr));
    exit;
}
...
$deleteClient = deleteActivityLog($clientSearchArr, $conn);
if (!noError($deleteClient)) {
    //error deleting client
    $logMsg = "Activity downlaod report  could not be deleted: {$deleteClient["errMsg"]}";
    ...
    $returnArr["errCode"] = 5;
    $returnArr["errMsg"] = "Error deleting Activity downlaod report. Please try again after some time.";
    echo(json_encode($returnArr));
    exit;
}

//Everything completed successfully
$logMsg = "Activity downlaod report deleted successfully. Return response";
...
$returnArr["errCode"] = -1;
$returnArr["errMsg"] = getErrMsg(-1)." Activity downlaod report deletion successful.";
echo(json_encode($returnArr));
exit;
?>