...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export spotify   data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export spotify   data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $emailSubject = "Generate export sppotify Report";
   ...
   // $youtube_report_table = 'youtube_video_claim_report_'.$year.'_'.$month;
    ...
    $allClientsInfocount  = ExportClientsSpotifymusicUnAssignedv2($youtube_report_table, $conn,$offset,$resultsPerPage,'',$youtube_report_table);
