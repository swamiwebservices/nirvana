...
if (!noError($conn)) {
    ...
    if (file_exists('../../../../excelreports/' . $clientsession . "_youtube_red_music_video_finance_report_" . $year . '_' . $month . '.zip')) {
        unlink('../../../../excelreports/' . $clientsession . "_youtube_red_music_video_finance_report_" . $year . '_' . $month . '.zip');
    }
    ...
    try {
        ...
        $allClientsInfocount = ExportClientsYoutuberedMusicre_finance_reportCountv3($table_type_name, $conn, $offset, $resultsPerPage, '', $clientsession);
        ...
        if (!noError($allClientsInfocount)) {
            ...
            if ($export) {
                ...
                if($type_table == "redmusic"){
                    $headtitle = ['country', 'contentType', 'channelID', 'videoID','AssetId', 'asset_title', 'asset_labels', 'Youtube payout', 'RevShare', 'USPAYOUT','Holding-Perc', 'WITHHOLDING', 'Final Payable','GST','Final Payable-GST', 'Channel'];
                } else {
                    $headtitle = ['country', 'contentType', 'channelID', 'videoID', 'videoTitle','AssetId', 'Youtube payout', 'RevShare', 'USPAYOUT', 'Holding-Perc', 'WITHHOLDING', 'Final Payable','GST','Final Payable-GST', 'Channel'];
                }
                ...
                for ($i = 0; $i < $noofexcels; $i++) {
                    ...
                    $allClientsInfo = ExportClientsYoutuberedMusicre_finance_reportv3($table_type_name, $conn, $offset, $resultsPerPage, $searchdata, $clientsession);
                ...
                $zip_file_tmp = '../../../../excelreports/' . $clientsession . '_youtube_red_music_video_finance_report_'.$type_table.'_' . $year . '_' . $month . '.zip';
                $zip = new ZipArchive();
        ...
        if (isset($argv[4])) {
            $zip_file_name = $clientsession . '_youtube_red_music_video_finance_report_'.$type_table.'_' . $year . '_' . $month . '.zip';
            $process_id = $argv[4];
