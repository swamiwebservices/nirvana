...
//type_table
    ...
    $allClientsInfocount  = ExportClientsItuneMusicReport_countv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession,$other_tables);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['Start Date','End Date','UPC','ISRC/ISBN','Vendor Identifier','Quantity','Partner Share','Extended Partner Share','Partner Share Currency','USD','Label Share','Sales or Returns','Apple Identifier','Artist/Show/Developer/Author','Title','Label/Studio/Network/Developer/Publisher','Grid','Product Type Identifier','ISAN/Other Identifier','Country Of Sale','Pre-order Flag','Promo Code','Customer Price','Customer Currency','FinalPayables','GST','Final Payable-GST'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsItuneMusicReportv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession,$other_tables);
