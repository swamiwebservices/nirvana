    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            if (!empty($search)) {
                ...
                where  {$v}.content_owner = '{$client}' and  video_title like '" . $search . "%'     )  ";
            } else {
                ...
                ( SELECT video_title FROM `{$other_tables}`    where  {$other_tables}.video_id = {$v}.videoID limit 1  ) as  video_title2  , {$rev_share} as rev_share, {$gst_percentage} as gst_percentage,  holding_percentage
    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            if ($pos === false) {
                ...
                Coalesce((SUM(CASE WHEN country='US' THEN partnerRevenue END)* {$holding_percentage}/100),0) as WITHHOLDING  ,'{$table_name}' as table_name , {$rev_share} as rev_share, {$gst_percentage} as gst_percentage, {$holding_percentage} as holding_percentage   FROM $v where  content_owner = '{$client}'  group by videoID )   ";
...
function ExportClientsYoutubeecompaidfeaturereportv2(
    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            $union_final_query[] = " ( SELECT purchase_type,channel_name,country,channel_id,video_id, earnings as partnerRevenue, Coalesce((CASE WHEN country='US' THEN partnerRevenue END),0) as USPAYOUT,
			Coalesce(((CASE WHEN country='US' THEN partnerRevenue END)*30/100),0) as WITHHOLDING ,'{$table_name}' as table_name , {$rev_share} as rev_share, {$gst_percentage} as gst_percentage, {$holding_percentage} as holding_percentage FROM $v where  content_owner = '{$client}' ) ";
        }
...
function ExportClientsYoutubeRedMusicReport_countv3(
    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            if (!empty($search)) {
                $check_query_total[] = " select  COUNT( DISTINCT videoID) as totalcount from   $v   WHERE {$v}.content_owner='{$client}' and  {$v}.video_title like '" . $search . "%'   ";

            } else {

                $check_query_total[] = " select  COUNT( DISTINCT videoID) as totalcount from   $v
                WHERE {$v}.content_owner='{$client}'    ";
    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            if (!empty($search)) {
				...
				Coalesce((SUM(CASE WHEN country='US' THEN partnerRevenue END)* {$holding_percentage}/100),0) as WITHHOLDING , '{$table_name}'  as tablename,  ( SELECT Channel FROM `channel_co_maping` ccm  where ccm.assetChannelID={$v}.assetChannelID limit 1 ) as channel_name ,
                ( SELECT record_label FROM `{$other_tables}`    where  {$other_tables}.asset_id = {$v}.assetID limit 1  ) as record_label, ( SELECT asset_title FROM `{$other_tables}`    where  {$other_tables}.asset_id = {$v}.assetID limit 1  ) as  asset_title, ( SELECT channel_display_name FROM `{$other_tables}`    where  {$other_tables}.asset_id = {$v}.assetID limit 1  ) as  channel_name2 ,
                ( SELECT video_title FROM `{$other_tables}`    where  {$other_tables}.video_id = {$v}.videoID limit 1  ) as  video_title2 , {$rev_share} as rev_share, {$gst_percentage} as gst_percentage, {$holding_percentage} as holding_percentage    FROM $v
                ...
                FROM $v where  {$v}.content_owner = '{$client}'   group by videoID )  ";
            }
        }
...
function ExportClientsSpotifyMusicReport_countv2(
    ...
    while ($row = mysqli_fetch_assoc($getClientInfoQueryResult["dbResource"])) {
        ...
        foreach ($row as $k => $v) {
            ...
            if (!empty($search)) {
                $check_query_total[] = " select  COUNT( DISTINCT Label) as totalcount from   $v   WHERE {$v}.content_owner='{$client}' and  {$v}.Label  like '" . $search . "%'   ";
                
            } else {

                $check_query_total[] = " select  COUNT( DISTINCT Label) as totalcount from   $v 
                WHERE {$v}.content_owner='{$client}'    ";
