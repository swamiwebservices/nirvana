...
if (!noError($conn)) {
    ...
    $logMsg = "Activate Apple background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Generate Activate Apple Report";
...
//type_table
    ...
    $allClientsInfocount  = ExportClientsAppleMusicReport_countv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession,$other_tables);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['Storefront Name' , ' Apple Identifier' , ' Membership Type' , ' Quantity' , ' Net Royalty' , ' Net Royalty Total' , ' USD' , ' Partner Share' , ' ISRC' , ' Item Title' , ' Item Artist' , ' Item Type' , ' Media Type' , ' Vendor Identifier' , ' Offline Indicator' , 'Label','Grid','Final Payable','GST','Final Payable-GST'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsAppleMusicReportv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession,$other_tables);
