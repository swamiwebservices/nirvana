...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export youtube finance data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export youtube finance data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $finance_report_table = 'youtube_red_finance_report_'.$year.'_'.$month;
    $youtube_report_table = 'youtube_red_video_report_'.$year.'_'.$month;
    ...
    if(file_exists('../../../../'.$clientsession."_Youtube_red_dashboard_".$year.'_'.$month.'.zip')){
        unlink('../../../../'.$clientsession."_Youtube_red_dashboard_".$year.'_'.$month.'.zip');
    }
    ...
    $allClientsInfocount  = ExportClientsYoutubeRedFinanceReport($finance_report_table, $youtube_report_table ,$conn,$offset,$resultsPerPage,'',$clientsession);
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['Video title','Channel name','AssetId','Youtube payout','RevShare','Final Payable'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsYoutubeRedFinanceReport($finance_report_table, $youtube_report_table ,$conn,$offset,$resultsPerPage,$searchdata,$clientsession);
                ...
                // //download the file
                $filename = "Youtube__Red_Client_data_".$year.'_'.$month.'_'.$i;
                $xcelfiles[]=$filename;
            ...
            $zip_file_tmp='../../../../'.$clientsession."_Youtube_Red_dashboard_".$year.'_'.$month.'.zip';
            $zip = new ZipArchive();
        ...
        $returnArr["errCode"] = -1;
         ...
         $emailMessage.='<br><a href="'.$rootUrl.$clientsession."_Youtube_Red_dashboard_".$year.'_'.$month.'.zip'.'">Click to Download </a>';
         $emailSubject = "SUCCESS: Youtube Red data is ready to download of ".$clientsession;
