...
if (!isset($argv) ) {
    session_start();
    $argv[] = "exportReportAmazon";
    $argv[] = "2020-07";
    $argv[] = $_SESSION["client"]; 
    }
//prepare for request
...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export Amazon report data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export Amazon report data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logMsg = "Activate Amazon report background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Generate Activate Amazon report Report";
   ...
   /// $file_path_export = "/var/www/html/laxman/content_repo_revamp/";
    $file_path_export = '../../../../';

    if(file_exists($file_path_export.$clientsession."_Amazon_dashboard_".$year.'_'.$month.'.zip')){
        unlink($file_path_export.$clientsession."_Amazon_dashboard_".$year.'_'.$month.'.zip');
    }
    ...
    $allClientsInfocount  = ExportClientsAmazonReport($finance_report_table, $youtube_report_table ,$conn,$offset,$resultsPerPage,'',$clientsession);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['Session Id','Video title','Royalty Amt','royaltyCurrency','Exchange Rate','Rev With Holding','With Holding','Amt Recieved','Client Share','Final Payable'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsAmazonReport($finance_report_table, $youtube_report_table ,$conn,$offset,$resultsPerPage,$searchdata,$clientsession);
            ...
            $logData["step8"]["data"] = "8.$i {$logMsg}";
             ...
             $zip_file_tmp=$file_path_export.$clientsession."_Amazon_dashboard_".$year.'_'.$month.'.zip';
        ...
        $returnArr["errCode"] = -1;
         ...
         $emailMessage.='<br><a href="'.$rootUrl.$clientsession."_Amazon_dashboard_".$year.'_'.$month.'.zip'.'">Click to Download </a>';
         $emailSubject = "SUCCESS: Amazon data is ready to download of ".$clientsession;
        ...
        //send error email
        $emailMessage = $importFailureEmailMessage."<p>Could not Exported Amazon data</p>". json_encode($logData);
        $emailSubject = "FAILURE: Amazon data is failed to download ";
        $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
