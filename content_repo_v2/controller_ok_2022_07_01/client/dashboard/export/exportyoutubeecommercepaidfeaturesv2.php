...
if (!noError($conn)) {
  ...
  if ($argv[3]) {
    ...
    if(file_exists('../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.'.zip')){
        unlink('../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.'.zip');
    }
    ...
    $allClientsInfocount  = ExportClientsYoutubeecompaidfeaturereportv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['country','purchase-type','channel-name','channel-id','video_id','Youtube payout','RevShare','USPAYOUT','Holding-Perc','WITHHOLDING','Final Payable','Channel','GST','Final Payable-GST'];
            ...
            for($i=0;$i<$noofexcels;$i++){
               ...
               $resultsPerPage = $noofexcels>1 ? $limit_total : $total;
                $allClientsInfo  = ExportClientsYoutubeecompaidfeaturereportv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession);
            ...
            $zip_file_tmp='../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.'.zip';
            $zip = new ZipArchive();
    ...
    if(isset($argv[4])){
        $zip_file_name = $clientsession."_youtube_ecommerce_paid_features_".$year."_".$month.".zip";
        $process_id = $argv[4];
        ...
        //end 
         ...
         $emailMessage.='<br><a href="'.$rootUrl.$clientsession."_Youtube_dashboard_".$year.'_'.$month.'.zip'.'">Click to Download </a>';
