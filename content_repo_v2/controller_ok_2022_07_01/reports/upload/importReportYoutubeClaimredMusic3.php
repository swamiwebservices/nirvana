 ...
 */
    ...
    if (!noError($insertInfoArr)) {
        ...
        $logMsg = "successfully imported csv into table. Run insertYoutubeVideoClaimReportInfo3 query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerChannelCOMapStep3(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerChannelCOMap for PP</p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
