...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the Spotify music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the Spotify music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
 $date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logFileName="importSpotifyMusicBackground.json";

    $logMsg = "Import spotify music background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import spotify music     Report";
   ...
   $loged_email_id = (isset($argv[3])) ? $argv[3] :'';
    ...
    $paramlog['file_name'] = "importReportSpotifyMusic.php";
    ...
    $insertInfoArr = insertSpotifyMusic_report_Table($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $logMsg = "successfully imported csv into table. Run spotify music query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerSpotifyCOMapStep1(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerItuneCOMapStep1</p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
