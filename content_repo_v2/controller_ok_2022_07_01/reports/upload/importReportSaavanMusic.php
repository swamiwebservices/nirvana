...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the saavan music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the saavan music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
 $date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logFileName="importSaavanMusicBackground.json";

    $logMsg = "Import saavan music background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import saavan music     Report";
   ...
   $loged_email_id = (isset($argv[3])) ? $argv[3] :'';
    ...
    $insertInfoArr = insertSaavanMusic_report_Table($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $paramlog['table_name'] = $tableName;
        $paramlog['file_name'] = "importReportSaavanMusic.php";
        $paramlog['status_name'] = "Import";
        ...
        $logMsg = "successfully imported csv into table. Run saavan music query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerSaavanCOMapStep1(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerSaavanCOMapStep1 </p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
