...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the Gaana music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the Gaana music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
 $date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logFileName="importGaanaMusicBackground.json";

    $logMsg = "Import Gaana music background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import Gaana music     Report";
   ...
   $loged_email_id = (isset($argv[3])) ? $argv[3] :'';
    ...
    $insertInfoArr = insertGaanaMusic_report_Table($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $paramlog['table_name'] = $tableName;
        $paramlog['file_name'] = "importReportGaanaMusic.php";
        $paramlog['status_name'] = "Import";
        ...
        $logMsg = "successfully imported csv into table. Run Gaana music query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerGaanaCOMapStep1(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerGaanaCOMapStep1</p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
