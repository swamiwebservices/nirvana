...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the itune music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the itune music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
 $date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logFileName="importItuneMusicBackground.json";

    $logMsg = "Import itune music background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import itune music     Report";
   ...
   $loged_email_id = (isset($argv[3])) ? $argv[3] :'';
    ...
    $insertInfoArr = insertItuneMusic_report_Table($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $paramlog['table_name'] = $tableName;
        $paramlog['file_name'] = "importReportItuneMusic.php";
        $paramlog['status_name'] = "Import";
        ...
        $logMsg = "successfully imported csv into table. Run itune music query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerItuneCOMapStep1(
            $tableName,
