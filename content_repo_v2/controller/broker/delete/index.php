...
$logFileName="deleteBroker.json";

$logMsg = "Delete broker process start.";
...
if (isset($_POST["brokerId"])) {
    $brokerId = cleanQueryParameter($conn, cleanXSS(urldecode($_POST["brokerId"])));
}

if (empty($brokerId)) {
    $logMsg = "Broker ID field empty: ".json_encode($_POST);;
    $logData["step2.1"]["data"] = "2.1. {$logMsg}";
    $logsProcessor->writeJSON($logFileName, $logFilePath, $logData, $initLogs["activity"]);

    $returnArr["errCode"] = 4;
    $returnArr["errMsg"] = getErrMsg(4)." Broker ID cannot be empty";
    echo(json_encode($returnArr));
    exit;
}
...
$logMsg = "Transaction started. Attempting to delete broker";
$logData["step4"]["data"] = "4. {$logMsg}";

//delete the broker
$brokerSearchArr = array(
    'broker_id'=>"'".$brokerId."'"
);

$deleteBroker = deleteBroker($brokerSearchArr, $conn);
if (!noError($deleteBroker)) {
    //error deleting broker
    $logMsg = "Broker could not be deleted: {$deleteBroker["errMsg"]}";
    ...
    $returnArr["errCode"] = 5;
    $returnArr["errMsg"] = "Error deleting broker. Please try again after some time.";
    echo(json_encode($returnArr));
    exit;
}

//Everything completed successfully
$logMsg = "Broker deleted successfully. Return response";
...
$returnArr["errCode"] = -1;
$returnArr["errMsg"] = getErrMsg(-1)." Broker deletion successful.";
echo(json_encode($returnArr));
exit;
?>