...
if (isset($_POST["distributorId"])) {
    $distributorId = cleanQueryParameter($conn, cleanXSS(urldecode($_POST["distributorId"])));
}

if (empty($distributorId)) {
    $logMsg = "Distributor ID field empty: ".json_encode($_POST);;
    $logData["step2.1"]["data"] = "2.1. {$logMsg}";
    $logsProcessor->writeJSON($logFileName, $logFilePath, $logData, $initLogs["activity"]);

    $returnArr["errCode"] = 4;
    $returnArr["errMsg"] = getErrMsg(4)." Distributor ID cannot be empty";
    echo(json_encode($returnArr));
    exit;
}
...
$distributorSearchArr = array(
    'distributor_id'=>"'".$distributorId."'"
);

$deleteDistributor = deleteDistributor($distributorSearchArr, $conn);
if (!noError($deleteDistributor)) {
    //error deleting Distributor
    $logMsg = "Distributor could not be deleted: {$deleteDistributor["errMsg"]}";
    ...
    $returnArr["errCode"] = 5;
    $returnArr["errMsg"] = "Error deleting distributor. Please try again after some time.";
    echo(json_encode($returnArr));
    exit;
}

//Everything completed successfully
$logMsg = "Distributor deleted successfully. Return response";
...
$returnArr["errCode"] = -1;
$returnArr["errMsg"] = getErrMsg(-1)." Distributor deletion successful.";
echo(json_encode($returnArr));
exit;
?>