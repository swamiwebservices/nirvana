...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export activation {$youtube_report_table}   data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export activation  {$youtube_report_table}  data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $emailSubject = "Generate Activate {$youtube_report_table} v2 Report";
...
//excelreports
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errCode"] = -1;
         ...
         $emailSubject = "SUCCESS: {$youtube_report_table} is ready to download of ".$youtube_report_table;
        ...
        //send error email
        $emailMessage = $importFailureEmailMessage."<p>Could not Exported {$youtube_report_table}</p>". json_encode($logData);
        $emailSubject = "FAILURE: {$youtube_report_table} is failed to download ";
        $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
