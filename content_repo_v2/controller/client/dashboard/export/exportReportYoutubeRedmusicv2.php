...
if (!noError($conn)) {
    ...
    if(file_exists('../../../../excelreports/'.$clientsession."_youtuberedmusic_".$year.'_'.$month.$downlaodType_zip. '.zip')){
        unlink('../../../../excelreports/'.$clientsession."_youtuberedmusic_".$year.'_'.$month.$downlaodType_zip. '.zip');
    }
    ...
    $table_type_name_file = "youtuberedmusic_video_report_redmusic";
    
    $table_type_name = "youtuberedmusic_video_report_redmusic\_";
    $table_type_name = "youtuberedmusic_video_report_redmusic%". $year . '_' . $month;
    $other_tables  =  "youtuberedmusic_video_report2_redmusic_". $year . '_' . $month;
                      //youtuberedmusic_video_report2_redmusic_
    $allClientsInfocount  = ExportClientsYoutubeRedMusicReport_countv3($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession,$other_tables,$downlaodType);
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            if($downlaodType == "withholding"){
                ...
                //,'USPAYOUT','Holding-Perc','WITHHOLDING','Final Payable','GST','Final Payable-with'
                $headtitle = ['Channel-Name','Content-Type','Label','Record-Label','asset_title','Channel-id','Video Id','Video title','ISRC','UPC','GRid','AssetId','assetChannelID','Youtube payout','RevShare','USPAYOUT','WITHHOLDING', 'Final Payable','GST','Final Payable-GST', 'Channel'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsYoutubeRedMusicReportv3($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession,$other_tables,$downlaodType);
                ...
                // //download the file
                $filename = $clientsession."_".$table_type_name_file.$year.'_'.$month.'_'.$i;
                $xcelfiles[]=$filename;
            ...
            $zip_file_tmp='../../../../excelreports/'.$clientsession."_youtuberedmusic_".$year.'_'.$month.$downlaodType_zip. '.zip';
            $zip = new ZipArchive();
    ...
    if(isset($argv[4])){
        $zip_file_name = $clientsession."_youtuberedmusic_".$year."_".$month.".zip";
        $process_id = $argv[4];
        ...
        //end 
         ...
         $emailMessage.='<br><a href="'.$rootUrl."excelreports/".$clientsession."_youtuberedmusic_".$year.'_'.$month.$downlaodType_zip. '.zip'.'">Click to Download </a>';
         $emailSubject = "SUCCESS: Youtube Red Music is ready to download of ".$clientsession;
        ...
        //send error email
        $emailMessage = $importFailureEmailMessage."<p>Could not Exported Youtube Red Music</p>". json_encode($logData);
        $emailSubject = "FAILURE: Youtube Red Music is failed to download ";
        $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
