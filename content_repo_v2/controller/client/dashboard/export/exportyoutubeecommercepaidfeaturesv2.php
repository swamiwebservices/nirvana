...
$downlaodType_zip = ($downlaodType=="withholding") ? $downlaodType : '';
    ...
    if(file_exists('../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.$downlaodType_zip .'.zip')){
        unlink('../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.$downlaodType_zip .'.zip');
    }
    ...
    $allClientsInfocount  = ExportClientsYoutubeecompaidfeaturereport_countv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession,$downlaodType);
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            if($downlaodType == "withholding"){
                $headtitle = [ 'channel-id', 'Youtube payout', 'RevShare', 'USPAYOUT','Holding-Perc', 'WITHHOLDING', 'Final Payable','GST','Final Payable-GST', 'Channel'];
            } else {
                //,'Holding-Perc', 'WITHHOLDING', 'Final Payable','GST','Final Payable-GST','USPAYOUT',
                $headtitle = ['country','purchase-type','channel-name','channel-id','video_id','Youtube payout','RevShare','USPAYOUT','WITHHOLDING','Final Payable','GST','Final Payable-GST','Channel'];
            }
            ...
            for($i=0;$i<$noofexcels;$i++){
               ...
               $resultsPerPage = $noofexcels>1 ? $limit_total : $total;
                $allClientsInfo  = ExportClientsYoutubeecompaidfeaturereportv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession,$downlaodType);
                ...
                // //download the file
                $filename = $clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.'_'.$i;
                $xcelfiles[]=$filename;
            ...
            $zip_file_tmp='../../../../excelreports/'.$clientsession."_youtube_ecommerce_paid_features_".$year.'_'.$month.$downlaodType_zip .'.zip';
            $zip = new ZipArchive();
    ...
    if(isset($argv[4])){
        $zip_file_name = $clientsession."_youtube_ecommerce_paid_features_".$year."_".$month.$downlaodType_zip .'.zip';
        $process_id = $argv[4];
...
//end
        ...
        //send error email
        $emailMessage = $importFailureEmailMessage."<p>Could not Exported Youtube ecommerce paid features report</p>". json_encode($logData);
        $emailSubject = "FAILURE: Youtube ecommerce paid features report is failed to download ";
        $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
