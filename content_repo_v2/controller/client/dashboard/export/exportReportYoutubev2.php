...
@unlink("poloaaaa.txt");
file_put_contents("poloaaaa.txt", $date1);
@chmod("poloaaaa.txt", 0777);
...
//print_r($argv);
    ...
    $finance_report_table = 'youtube_video_claim_report_' . $year . '_' . $month;
    ...
    @unlink("clientsession.txt");
    file_put_contents("clientsession.txt", $clientsession);
    @chmod("clientsession.txt", 0777);
    ...
    if (file_exists('../../../../excelreports/' . $clientsession."_YoutubeClaim_report_" . $year . '_' . $month .$downlaodType_zip. '.zip')) {
        $zip_file_tmp = '../../../../excelreports/' . $clientsession."_YoutubeClaim_report_" . $year . '_' . $month.$downlaodType_zip . '.zip';
    }
    ...
    try {
        ...
        if (isset($argv[4])) {
            $zip_file_name =  $clientsession."_YoutubeClaim_report_" . $year . "_" . $month.$downlaodType_zip . ".zip";
            $process_id = $argv[4];
            $process_date_start = date("Y-m-d H:i:s");
            $sql_update = "update activity_downlaod_report  set process_date_start='{$process_date_start}',  status_flag = '5', status_message = 'Process-started'     where id='{$process_id}'";
        ...
        $table_type_name = "youtube_video_claim_report\_";
        $table_type_name = "youtube_video_claim_report_nd%" . $year . '_' . $month;
        $other_tables  =  "youtube_video_claim_report2_nd%". $year . '_' . $month;
        $allClientsInfocount = ExportClientsYoutubeClaimReportCountv3($table_type_name, $conn, $offset, $resultsPerPage, '', $clientsession,$downlaodType);
        ...
        if (!noError($allClientsInfocount)) {
            ...
            if ($export) {
                ...
                if($downlaodType == "withholding"){
                    ...
                    //,'Holding-Perc', 'WITHHOLDING', 'Final Payable','GST','Final Payable-GST','USPAYOUT',
                    $headtitle = ['Country','channel-Name', 'channel-ID', 'video-ID', 'Video title','Content-Type','Label','asset_title', 'AssetId', 'assetChannelID', 'Youtube payout', 'RevShare','USPAYOUT','WITHHOLDING', 'Final Payable','GST','Final Payable-GST', 'Channel'];
                }
                ...
                for ($i = 0; $i < $noofexcels; $i++) {
                    ...
                    $allClientsInfo = ExportClientsYoutubeClaimReportv3($table_type_name, $conn, $offset, $resultsPerPage, $searchdata, $clientsession,$other_tables,$downlaodType);
                    ...
                    // //download the file
                    $filename =  $clientsession."_YoutubeClaim_report_" . $year . '_' . $month . '_' . $i;
                    $xcelfiles[] = $filename;
        ...
        //unset the processid as it is completed or ended
        ...
        $emailMessage .= '<br><a href="' . $rootUrl . "excelreports/".$clientsession."_Youtube_dashboard_" . $year . '_' . $month .$downlaodType_zip. '.zip' . '">Click to Download </a>';
