...
$importFailureEmailMessage = "<div>
    <p>There was an error in exportReportRedSubscription_non_cmgv2.php on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error exportReportRedSubscription_non_cmgv2.php into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logMsg = "Export red-subscription_NON_CMG background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Generate Export red-subscription_NON_CMG Report";
...
//type_table
    ...
    if(file_exists('../../../../excelreports/'.$clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'.zip')){
        unlink('../../../../excelreports/'.$clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'.zip');
    }
 ...
 //   $table_type_name_file = 'report_publishing_'.$type_table;
    
     
    $table_type_name = 'report_publishing_'.$type_table.'_'.$year.'_'.$month;
    $other_tables  =  'report_publishing_'.$type_table.'_'.$year.'_'.$month;
                     
    $allClientsInfocount  = ExportClientReport_main_non_CMG_countv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession,$other_tables);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['Country','Video_ID','Channel_ID','Asset_ID','Asset_Label','Asset_Channel_ID','Asset_Type','Partner_Revenue'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                @unlink("polo_export_noofexcels".$i.".txt");
                file_put_contents("polo_export_noofexcels".$i.".txt", $total."_".$noofexcels."_".$offset."_".$resultsPerPage);
                @chmod("polo_export_noofexcels".$i.".txt", 0777);

                
                $allClientsInfo  = ExportClientReport_red_subscription_non_CMGv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession,$other_tables);
                ...
                // //download the file
                $filename = $clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'_'.$i;
                $xcelfiles[]=$filename;
            ...
            $zip_file_tmp='../../../../excelreports/'.$clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'.zip';
            $zip = new ZipArchive();
    ...
    if(isset($argv[4])){
        $zip_file_name = $clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'.zip';
        $process_id = $argv[4];
        ...
        //end 
         ...
         $emailMessage.='<br><a href="'.$rootUrl.'excelreports/'.$clientsession.'_report_publishing_'.$type_table.'_'.$year.'_'.$month.'.zip'.'">Click to Download </a>';
