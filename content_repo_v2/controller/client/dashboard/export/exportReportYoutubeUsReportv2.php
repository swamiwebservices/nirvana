...
$importFailureEmailMessage = "<div>
    <p>There was an error in Export youtube  us report  data on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error Export youtube us report   data into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
  ...
  $clientsession = base64_decode($clientsession);
    ...
    $allClientsInfocount  = ExportClientsYoutubeUSReport_countv2($table_type_name ,$conn,$offset,$resultsPerPage,'',$clientsession);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         if ($export) {
            ...
            $xcelfiles = [];
            $headtitle = ['AssetId','Youtube payout','RevShare','Final Payable','GST','Final Payable-GST'];
            ...
            for($i=0;$i<$noofexcels;$i++){
                ...
                $allClientsInfo  = ExportClientsYoutubeUSReportv2($table_type_name, $conn,$offset,$resultsPerPage,$searchdata,$clientsession);
                ...
                // //download the file
                $filename = $clientsession."_youtubusreport_".$type_table."_".$year.'_'.$month.'_'.$i;
                $xcelfiles[]=$filename;
            ...
            $zip_file_tmp='../../../../excelreports/'.$clientsession."_youtubusreport_".$type_table."_".$year.'_'.$month.'.zip';
            $zip = new ZipArchive();
    ...
    if(isset($argv[4])){
        $zip_file_name = $clientsession."_youtubusreport_".$type_table."_".$year."_".$month.".zip";
        $process_id = $argv[4];
