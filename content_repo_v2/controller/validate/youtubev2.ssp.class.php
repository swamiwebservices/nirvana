...
class SSP
{
     ...
     */
    public static function simple($request, $conn, $table, $primaryKey, $columns, $searchFilter = array())
    {
        $bindings = array();
        $db = self::db($conn);
        ...
        if (!empty($searchFilter['content_owner'])) {

			$sqlWhere_temp = (strpos($where, 'WHERE') !== false) ? " AND " : " WHERE ";
			
            if ($searchFilter['content_owner'] == "assigned") {
                $sqlWhere = $sqlWhere_temp ."   (  content_owner!='' and content_owner IS NOT NULL) ";
            } else {
                $sqlWhere = $sqlWhere_temp. "   (content_owner IS NULL || content_owner='') ";
            }
