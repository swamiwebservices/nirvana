...
if (!noError($conn)) {
   ...
   // $youtube_report_table = 'youtube_video_claim_report_'.$year.'_'.$month;
    ...
    $allClientsInfocount  = ExportClientsSaavanmusicUnAssignedv2($youtube_report_table, $conn,$offset,$resultsPerPage,'',$youtube_report_table);
    ...
    if (!noError($allClientsInfocount)) {
        ...
        $returnArr["errMsg"] = getErrMsg(5)." Error fetching clients details.";
         ...
         $emailMessage.='<br>';
         $emailSubject = "SUCCESS: gaana export is ready to download of ".$youtube_report_table;
        ...
        //send error email
        $emailMessage = $importFailureEmailMessage."<p>Could not Exported gaana export</p>". json_encode($logData);
        $emailSubject = "FAILURE: gaana export is failed to download ";
        $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
