...
$logMsg = "Delete group process start.";
...
if (isset($_POST["groupId"])) {
    $groupId = cleanQueryParameter($conn, cleanXSS(urldecode($_POST["groupId"])));
}

if (empty($groupId)) {
    $logMsg = "Group ID field empty: ".json_encode($_POST);;
    $logData["step2.1"]["data"] = "2.1. {$logMsg}";
    $logsProcessor->writeJSON($logFileName, $logFilePath, $logData, $initLogs["activity"]);

    $returnArr["errCode"] = 4;
    $returnArr["errMsg"] = getErrMsg(4)." Group ID cannot be empty";
    echo(json_encode($returnArr));
    exit;
}
...
$logMsg = "Transaction started. Attempting to delete group";
$logData["step4"]["data"] = "4. {$logMsg}";

//delete the group
$groupSearchArr = array( 
    'group_id'=>"'".$groupId."'"
);
$deletegroup = deleteGroupInfo($groupSearchArr, $conn);
if (!noError($deletegroup)) {
    //error deleting group
    $logMsg = "Group could not be deleted: {$deleteGroup["errMsg"]}";
    ...
    $returnArr["errCode"] = 5;
    $returnArr["errMsg"] = "Error deleting group. Please try again after some time.";
    echo(json_encode($returnArr));
    exit;
}

//Everything completed successfully
$logMsg = "Group deleted successfully. Return response";
...
$returnArr["errCode"] = -1;
$returnArr["errMsg"] = getErrMsg(-1)." Group deletion successful.";
echo(json_encode($returnArr));
exit;
?>