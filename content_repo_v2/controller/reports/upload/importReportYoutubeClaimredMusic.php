 ...
 */
    ...
    $emailSubject = "Import Youtube video claim Report";
    ...
    $insertInfoArr = insertYoutubeRedMusicVideoReportInfo($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $logMsg = "successfully imported csv into table. Run insertYoutubeVideoClaimReportInfo query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerChannelCOMapStep1(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            $rollback = rollbackTransaction($conn);
            $logMsg = "Could not run autoAssignChannelCOMap for PP: {$tableName}".json_encode($autoAssignChannelCOMap);
            $logData["step6.1"]["data"] = "6.1. {$logMsg}";

            file_put_contents("testprocesses.php", json_encode($logData). "\n", FILE_APPEND);

            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerChannelCOMapStep1 for PP</p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
