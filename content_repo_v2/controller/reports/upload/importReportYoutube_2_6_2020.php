...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the Youtube Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the Youtube Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logMsg = "Import Youtube background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import Youtube Report";
    ...
    $logMsg = "Transaction started. Inserting csv report into table";
    $logData["step5"]["data"] = "5. {$logMsg}";

    $insertInfoArr = insertReportInfo($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $logMsg = "successfully imported csv into table. Run autoAssignChannelCOMap query for partner-provided: ";
        $logData["step6"]["data"] = "6. {$logMsg}";

        $fieldSearchArr = array("LOWER(contentType)"=>"partner-provided");
        $contentType = "partner_provided";
        $autoAssignChannelCOMap = autoAssignChannelCOMap(
            $tableName,
            $fieldSearchArr,
            $contentType,
            $conn
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            $logMsg = "successfully run autoAssignChannelCOMap for PP. Run autoAssignChannelCOMap query for UGC: ";
            $logData["step8"]["data"] = "8. {$logMsg}";

            $fieldSearchArr = array("LOWER(contentType)"=>"ugc");
            $contentType = "ugc";
            $autoAssignChannelCOMap = autoAssignChannelCOMap(
                $tableName,
                $fieldSearchArr,
            ...
            if (!noError($autoAssignChannelCOMap)) {
                $rollback = rollbackTransaction($conn);
                $logMsg = "Could not run autoAssignChannelCOMap for UGC: ".json_encode($autoAssignChannelCOMap);
                $logData["step6.1"]["data"] = "6.1. {$logMsg}";

                file_put_contents("testprocesses.php", json_encode($logData). "\n", FILE_APPEND);

                //send error email
                $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignChannelCOMap for UGC</p>";
                $emailSubject = "FAILURE: ".$emailSubject;
                $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
                if (!noError($sendEmail)) {
                    //error sending email
                ...
                $logMsg = "successfully run autoAssignChannelCOMap for UGC. Run auto assign queries upto 6 months back: ";
                $logData["step9"]["data"] = "9. {$logMsg}";

                $errorsOccurred = array();
                for ($i=1; $i<=1; $i++) {
                    $destMonthYear = explode("_", str_replace("youtube_report_", "", $tableName));
                    $destMonthYear = "01-".$destMonthYear[1]."-".$destMonthYear[0];
                    $sourceMonthYear = date("Y_m", strtotime("-{$i} months", strtotime($destMonthYear)));
                    $sourceTableName = "youtube_report_".$sourceMonthYear;

                    $logMsg = "Run auto assign queries {$i} months ago: ".$sourceTableName;
                    $logData["step9.{$i}"]["data"] = "9.{$i} {$logMsg}";
                    
                    $autoAssignPrevMonths = autoAssignPrevMonths(
                        $tableName,
                        $sourceTableName,
                        $conn
                    );
                    if (!noError($autoAssignPrevMonths)) {
                       //  $rollback = rollbackTransaction($conn);
                        ...
                        $logMsg = "successfully ran autoAssignPrevMonths for {$i}: ".json_encode($autoAssignPrevMonths);
                        $logData["step9.{$i}.1"]["data"] = "9.{$i}.1. {$logMsg}";
                    }
                }
				...
				//	echo $date2->h.' hours'."\n";
					$importtime =  $date2->i.' minutes '.$date2->s.' seconds';
					 
                    //send success email
                    $emailMessage = $importSuccessEmailMessage."<p>Successfully imported csv in ".$importtime." </p>".$filePath;
