...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the apple music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the apple music Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
 $date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logFileName="importAppleMusicBackground.json";

    $logMsg = "Import apple music background process start: ".date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import apple music     Report";
   ...
   $loged_email_id = (isset($argv[3])) ? $argv[3] :'';
    ...
    $insertInfoArr = insertAppleMusic_report_Table($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $paramlog['table_name'] = $tableName;
        $paramlog['file_name'] = "importReportAppleMusic.php";
        $paramlog['status_name'] = "Import";
        ...
        $logMsg = "successfully imported csv into table. Run apple music query for partner-provided: ".'_____'.date("h:m:s");
        ...
        $autoAssignChannelCOMap = autoAssignContentOwnerAppleCOMapStep1(
            $tableName,
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            //send error email
            $emailMessage = $importFailureEmailMessage."<p>Could not run autoAssignContentOwnerAppleCOMapStep1 for PP</p>";
            $emailSubject = "FAILURE: ".$emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
