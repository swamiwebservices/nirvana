...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the Youtube Audio claim v1.1 Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the Youtube Audio claim v1.1 Report into Content Reporting on ".date("Y-m-d")." at ".date("h:m:s")."</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    //get the user info
    $email = "importYoutubeAudioFinanceReport@background.process";
    ...
    $emailSubject = "Import Youtube Audio Claim v1.1";
    ...
    $insertInfoArr = insertYRAudioFinanceReportInfo($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        $autoAssignChannelCOMap = autoAssignChannelCOMapforYTAudio(
            $tableName,
            $fieldSearchArr,
            $contentType,
            $conn
        ...
        if (!noError($autoAssignChannelCOMap)) {
            ...
            $logMsg = "successfully run autoAssignChannelCOMap for PP. Run autoAssignChannelCOMap query for UGC: ".'_____'.date("h:m:s");
            ...
            $autoAssignChannelCOMap = autoAssignChannelCOMapforYTAudio(
             ...
             if (!noError($autoAssignChannelCOMap)) {
                ...
                $logMsg = "successfully run autoAssignChannelCOMap for UGC. Run auto assign queries upto 6 months back: ".'_____'.date("h:m:s");
                $logData["step9"]["data"] = "9. {$logMsg}";

                $errorsOccurred = array();
                // for ($i=1; $i<=6; $i++) {
                //     $destMonthYear = explode("_", str_replace("youtube_finance_report_", "", $tableName));
