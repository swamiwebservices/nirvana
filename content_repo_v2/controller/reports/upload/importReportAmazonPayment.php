...
$importFailureEmailMessage = "<div>
    <p>There was an error importing the Amazon   Payment Report  into Content Reporting on " . date("Y-m-d") . " at " . date("h:m:s") . "</p>
</div>";
$importSuccessEmailMessage = "<div>
    <p>There was NO error importing the Amazon   Payment Report  into Content Reporting on " . date("Y-m-d") . " at " . date("h:m:s") . "</p>
</div>";
$date1 = new DateTime();
...
if (!noError($conn)) {
    ...
    $logMsg = "Import Amazon Payment Report  background process start: " . date("Y-m-d h:i:s");
    ...
    $emailSubject = "Import Amazon Payment Report";
    ...
    $insertInfoArr = insertReportAmazonVideoPInfoPayment($filePath, $tableName, $conn);
    ...
    if (!noError($insertInfoArr)) {
        ...
        if (!noError($sendEmail)) {
            //error sending email
            $logMsg = "Mail not sent";
            $logData["step6.1"]["data"] = "5.2. {$logMsg}";
            file_put_contents("testprocesses.php", json_encode($logData) . "\n", FILE_APPEND);
            exit;
        }
        exit;
        ...
        $logMsg = "successfully imported csv into table. Run autoCalculateWithHolding query for withholding cal: " . '_____' . date("h:m:s");
        ...
        $paramlog['table_name'] = $tableName;
        $paramlog['file_name'] = '';
        $paramlog['status_name'] = "Auto-Calculating";
        $paramlog['status_flag'] = "start";
        $paramlog['date_added'] = date("Y-m-d H:i:s");
        $paramlog['ip_address'] = get_client_ip();
        $paramlog['login_user'] = $_SESSION["userEmail"];
        ...
        $autoCalculateWithHolding = autoCalculateWithHolding(
            $tableName,
           
            $conn
        );
        if (!noError($autoCalculateWithHolding)) {
            $rollback = rollbackTransaction($conn);
            $logMsg = "Could not run autoCalculateWithHolding for PP: " . json_encode($autoCalculateWithHolding);
            $logData["step6.1"]["data"] = "6.1. {$logMsg}";

            file_put_contents("testprocesses.php", json_encode($logData) . "\n", FILE_APPEND);

            //send error email
            $emailMessage = $importFailureEmailMessage . "<p>Could not run autoCalculateWithHolding for PP</p>";
            $emailSubject = "FAILURE: " . $emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
                $logMsg = "Mail not sent";
                $logData["step6.1"]["data"] = "6.2. {$logMsg}";
                file_put_contents("testprocesses.php", json_encode($logData) . "\n", FILE_APPEND);
                exit;
            }
            exit;
            ...
            $logMsg = "successfully run  autoCalculateWithHolding .  : " . '_____' . date("h:m:s");
            $logData["step6"]["data"] = "7. {$logMsg}";
            ...
            //send success email
            $emailMessage = $importSuccessEmailMessage . "<p>Successfully auto Calculate With Holding csv in " . $importtime . " </p>Imported files :<br/>" . $filePath;
            $emailSubject = "SUCCESS: " . $emailSubject;
            $sendEmail = sendMail(IMPORTNOTIFIERS, $emailSubject, $emailMessage);
            if (!noError($sendEmail)) {
                //error sending email
                $logMsg = "Mail not sent";
                $logData["step6.1"]["data"] = "7.3. {$logMsg}";
                file_put_contents("testprocesses.php", json_encode($logData) . "\n", FILE_APPEND);
                exit;
            }
            exit;
